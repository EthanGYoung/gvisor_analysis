...
def runExperiment(TRIALS, WRITE_SIZE, path):
	...
	for i in range(0, 10):
		...
		flush()
		p = Popen(['/bin/bash', '-c',  str(path) + "/experiment_materials/write " + str(TRIALS) + " " +  str(exp_write)])
		p.wait()
		flush()
...
runExperiment(TRIALS, WRITE_SIZE, path)
